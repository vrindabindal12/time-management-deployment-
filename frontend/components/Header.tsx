  ...
  return (
    <header className="sticky top-0 z-40 glass-panel border-b border-white/60">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center py-4">
          ...
          <div className="lg:hidden flex items-center">
            <button 
              ...
              className="p-2 text-slate-600 hover:text-slate-900 hover:bg-white/50 rounded-lg transition-all focus:outline-none"
        ...
        {mobileMenuOpen && (
          <div className="lg:hidden animate-in slide-in-from-top-2 fade-in duration-200 pb-4">
            <div className="flex flex-col space-y-2 border-t border-white/50 pt-4">
              {navItems.map((item) => (
                <Link
                  key={item.path}
                  href={item.path}
                  className={`px-4 py-3 rounded-xl text-sm font-bold transition-all duration-200 ${pathname === item.path
                    ? 'bg-blue-100/50 text-blue-900 shadow-sm border border-blue-200/50'
              ...
              {user && (
                <div className="mt-4 pt-4 border-t border-white/50 flex items-center justify-between px-2">
                  <div className="flex flex-col items-start gap-1">
                    <p className="text-slate-900 text-sm font-semibold leading-none">{user.name}</p>
                    
                    {/* Role display / switcher for Mobile */}
                    {isBoth ? (
                      <div className="flex items-center gap-2 mt-1">
                        <button
                          onClick={() => handleRoleSwitch('admin')}
                          className={`px-3 py-1.5 rounded-full text-[11px] font-bold transition-all duration-200 ${activeRole === 'admin' 
                            ? 'bg-gradient-to-r from-violet-500 to-indigo-600 text-white shadow-md' 
                        ...
                        <button
                          onClick={() => handleRoleSwitch('employee')}
                          className={`px-3 py-1.5 rounded-full text-[11px] font-bold transition-all duration-200 ${activeRole === 'employee' 
                            ? 'bg-gradient-to-r from-emerald-500 to-teal-600 text-white shadow-md' 
                            : 'bg-white/60 text-slate-600 hover:bg-white border border-slate-200'}`}
                        >
                          Employee
                      ...
                      <span className={`px-2.5 py-0.5 rounded-full text-[11px] font-bold shadow-sm ${roleBadgeColor} mt-1`}>
                        {roleLabel}
                      </span>
                    )}
                  </div>
                  ...
                  <button
                    onClick={logout}
                    className="glass-danger-btn hover:brightness-95 text-white px-4 py-2 rounded-xl text-sm font-bold transition-all duration-200 shadow-md"
                  >
                    Logout
                  </button>
                </div>
